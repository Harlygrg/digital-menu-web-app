...
class CustomerProvider extends ChangeNotifier {
  ...
  Future<void> initialize() async {
    try {
      ...
      if (_customerId != null) {
        appDebugLog('CustomerProvider: Found saved customer ID: $_customerId');
      } else {
        appDebugLog('CustomerProvider: No saved customer ID found');
      }
  ...
  Future<int?> addCustomer({
    ...
    try {
      ...
      appDebugLog(
        'CustomerProvider: Adding customer - name: $name, phone: $phone',
      );
      ...
      appDebugLog(
        'CustomerProvider: Customer added successfully - ID: $customerId',
      );

      notifyListeners();
      return customerId;
    } catch (e) {
      appDebugLog('CustomerProvider: Error adding customer: $e');
  ...
  Future<void> _ensureAuthentication() async {
    try {
      ...
      if (accessToken != null && accessToken.isNotEmpty) {
        appDebugLog('CustomerProvider: Access token already exists');
        return;
      }
      ...
      try {
        ...
        if (fcmToken != null && fcmToken.isNotEmpty) {
          ...
          appDebugLog(
            'CustomerProvider: FCM token is empty (notification permissions may not be granted)',
          );
          fcmToken = ''; // Use empty string if token is not available
        }
      } catch (e) {
        appDebugLog(
          'CustomerProvider: Error fetching FCM token from Firebase: $e',
        );
        fcmToken = ''; // Use empty string on error
      }

      // Register as guest user to get tokens
      final response = await GuestUserApi.registerGuestUser(deviceId);

      if (response.success && response.data?.accessToken != null) {
        appDebugLog('CustomerProvider: Guest user registration successful');
      } else {
        throw Exception('Failed to register guest user');
      }
    } catch (e) {
      appDebugLog('CustomerProvider: Error during authentication: $e');
  ...
  Future<String> _getDeviceId() async {
    try {
      ...
      appDebugLog('CustomerProvider: Generated device ID: $deviceId');
      return deviceId;
    } catch (e) {
      appDebugLog('CustomerProvider: Error getting device ID: $e');
  ...
  Future<void> clearCustomer() async {
    try {
      appDebugLog('CustomerProvider: Clearing customer data');

      await _repository.clearCustomerId();
      _customerId = null;
      _errorMessage = null;

      appDebugLog('CustomerProvider: Customer data cleared');

      notifyListeners();
    } catch (e) {
      appDebugLog('CustomerProvider: Error clearing customer: $e');
    }
  }
  ...
  Future<void> refreshCustomerId() async {
    try {
      appDebugLog('CustomerProvider: Refreshing customer ID');

      _customerId = await _repository.getCustomerId();

      appDebugLog('CustomerProvider: Customer ID refreshed: $_customerId');

      notifyListeners();
    } catch (e) {
      appDebugLog('CustomerProvider: Error refreshing customer ID: $e');
    }
  }
  ...
  String _parseErrorMessage(String error) {
    appDebugLog('CustomerProvider: Parsing error: $error');
    ...
    } else if (error.contains('Invalid access token') ||
  ...
  Future<bool> needsRegistration() async {
    try {
      ...
      appDebugLog('CustomerProvider: Error checking registration status: $e');
