...
class NotificationService {
  ...
  Future<String?> initialize({String? vapidKey, BuildContext? context}) async {
    if (_isInitialized) {
      if (kDebugMode) {
        appDebugLog('⚠️ NotificationService already initialized');
      }
    ...
    try {
      if (kDebugMode) {
        appDebugLog('🔔 Initializing Firebase Messaging...');
      }
      ...
      if (kIsWeb) {
        ...
        if (kDebugMode) {
          appDebugLog(
            '🔍 Checking browser permission (live): $browserPermission',
          );
        }
        ...
        if (browserPermission == 'granted') {
          ...
          if (kDebugMode) {
            appDebugLog('✅ Permission already granted, skipping request');
          }
          permission = true;
        } else if (browserPermission == 'denied') {
          // Permission explicitly denied by user
          if (kDebugMode) {
            appDebugLog('❌ Permission denied by user in browser settings');
          }
          return null;
        } else {
          // Permission not yet requested (default state), request it now
          if (kDebugMode) {
            appDebugLog('📱 Permission not yet requested, requesting now...');
          }
      ...
      if (!permission) {
        if (kDebugMode) {
          appDebugLog('❌ Notification permission denied');
        }
        return null;
      }
      ...
      if (kIsWeb && vapidKey != null) {
        _currentToken = await _firebaseMessaging.getToken(vapidKey: vapidKey);
      } else {
      ...
      if (_currentToken != null) {
        if (kDebugMode) {
          appDebugLog('✅ FCM Token obtained: $_currentToken');
        }
        ...
        if (kDebugMode) {
          appDebugLog('❌ Failed to obtain FCM token');
        }
      }
      ...
      if (kDebugMode) {
        appDebugLog('✅ Firebase Messaging initialized successfully');
      }

      return _currentToken;
    } catch (e) {
      if (kDebugMode) {
        appDebugLog('❌ Error initializing Firebase Messaging: $e');
      }
      return null;
    }
  }
  ...
  String getBrowserNotificationPermission() {
    if (kIsWeb) {
      try {
        ...
        appDebugLog('🔍 Browser notification permission (live): $permission');
        return permission ?? 'default';
      } catch (e) {
        appDebugLog('⚠️ Error getting browser notification permission: $e');
        return 'default';
      }
    }
  ...
  Future<bool> requestPermission() async {
    try {
      appDebugLog('📱 Requesting notification permission from browser...');
      ...
      appDebugLog('🔔 Permission status: ${settings.authorizationStatus}');
      appDebugLog(
        '🔔 Permission details: alert=${settings.alert}, badge=${settings.badge}, sound=${settings.sound}',
      );
      ...
      appDebugLog(granted ? '✅ Permission GRANTED' : '❌ Permission DENIED');

      return granted;
    } catch (e, stackTrace) {
      appDebugLog('❌ Error requesting permission: $e');
      appDebugLog('📍 Stack trace: $stackTrace');
      return false;
    }
  }
  ...
  void _setupTokenRefreshListener() {
    _firebaseMessaging.onTokenRefresh.listen(
      (newToken) async {
        if (kDebugMode) {
          appDebugLog('🔄 FCM Token refreshed: $newToken');
        }
      ...
      onError: (error) {
        if (kDebugMode) {
          appDebugLog('❌ Error on token refresh: $error');
        }
      },
    );
  }
  ...
  void _setupForegroundMessageHandler() {
    FirebaseMessaging.onMessage.listen(
      ...
      onError: (error) {
        if (kDebugMode) {
          appDebugLog('❌ Error receiving foreground message: $error');
        }
      },
    );
  }
  ...
  void _setupBackgroundMessageHandler() {
    ...
    FirebaseMessaging.onMessageOpenedApp.listen(
      (RemoteMessage message) {
        if (kDebugMode) {
          appDebugLog('📨 Background notification clicked:');
          appDebugLog('  Title: ${message.notification?.title}');
          appDebugLog('  Body: ${message.notification?.body}');
          appDebugLog('  Data: ${message.data}');
        }
      ...
      onError: (error) {
        if (kDebugMode) {
          appDebugLog('❌ Error handling background notification click: $error');
        }
      },
    );
  }
  ...
  Future<void> _checkInitialMessage() async {
    try {
      ...
      if (initialMessage != null) {
        if (kDebugMode) {
          appDebugLog('📨 App opened from notification (terminated state):');
          appDebugLog('  Title: ${initialMessage.notification?.title}');
          appDebugLog('  Body: ${initialMessage.notification?.body}');
          appDebugLog('  Data: ${initialMessage.data}');
        }
      ...
      if (kDebugMode) {
        appDebugLog('❌ Error checking initial message: $e');
      }
    }
  }
    ...
    try {
      ...
      html.window.navigator.serviceWorker?.addEventListener('message', (event) {
        ...
        if (data is Map && data['type'] == 'NOTIFICATION_CLICK') {
          if (kDebugMode) {
            appDebugLog(
              '📨 Service worker notification click message received',
            );
            appDebugLog('  URL: ${data['url']}');
          }
      ...
      if (kDebugMode) {
        appDebugLog('✅ Service worker listener setup complete');
      }
    } catch (e) {
      if (kDebugMode) {
        appDebugLog('❌ Error setting up service worker listener: $e');
      }
    }
  }
  ...
  void _showForegroundNotificationPopup(RemoteMessage message) {
    if (_context == null || !_context!.mounted) {
      if (kDebugMode) {
        appDebugLog('⚠️ Cannot show popup: context not available');
      }
      return;
    }
  ...
  void _navigateToOrderTracking() {
    if (_context == null || !_context!.mounted) {
      if (kDebugMode) {
        appDebugLog('⚠️ Cannot navigate: context not available');
      }
    ...
    try {
      if (kDebugMode) {
        appDebugLog('🚀 Navigating to order tracking screen...');
      }
      ...
      if (kDebugMode) {
        appDebugLog('✅ Navigation completed');
      }
    } catch (e) {
      if (kDebugMode) {
        appDebugLog('❌ Error navigating: $e');
      }
    }
  }
  ...
  Future<String?> refreshToken({String? vapidKey}) async {
    try {
      if (kDebugMode) {
        appDebugLog('🔄 Refreshing FCM token...');
      }
      ...
      if (_currentToken != null) {
        if (kDebugMode) {
          appDebugLog('✅ Token refreshed: $_currentToken');
        }
        _tokenStreamController.add(_currentToken!);
  ...
  Future<void> deleteToken() async {
    try {
      if (kDebugMode) {
        appDebugLog('🗑️ Deleting FCM token...');
      }
      ...
      if (kDebugMode) {
        appDebugLog('✅ FCM token deleted');
      }
    } catch (e) {
      if (kDebugMode) {
        appDebugLog('❌ Error deleting token: $e');
      }
    }
  }
  ...
  Future<void> subscribeToTopic(String topic) async {
    try {
      if (kDebugMode) {
        appDebugLog('📢 Subscribing to topic: $topic');
      }

      await _firebaseMessaging.subscribeToTopic(topic);

      if (kDebugMode) {
        appDebugLog('✅ Subscribed to topic: $topic');
      }
    } catch (e) {
      if (kDebugMode) {
        appDebugLog('❌ Error subscribing to topic: $e');
      }
    }
  }
  ...
  Future<void> unsubscribeFromTopic(String topic) async {
    try {
      if (kDebugMode) {
        appDebugLog('📢 Unsubscribing from topic: $topic');
      }

      await _firebaseMessaging.unsubscribeFromTopic(topic);

      if (kDebugMode) {
        appDebugLog('✅ Unsubscribed from topic: $topic');
      }
    } catch (e) {
      if (kDebugMode) {
        appDebugLog('❌ Error unsubscribing from topic: $e');
      }
    }
  }
  ...
  Future<String> getFcmToken() async {
    try {
      appDebugLog('🔍 getFcmToken: Starting...');

      if (_currentToken != null && _currentToken!.isNotEmpty) {
        appDebugLog(
          '✅ getFcmToken: Using cached token (${_currentToken!.length} chars)',
        );
        return _currentToken!;
      }

      appDebugLog('⏳ getFcmToken: No cached token, requesting permission...');

      final permission = await requestPermission();
      appDebugLog('🔐 getFcmToken: Permission result: $permission');

      if (!permission) {
        appDebugLog('❌ getFcmToken: Notification permission denied by user');
        return '';
      }

      appDebugLog(
        '📡 getFcmToken: Permission granted, requesting token from FCM...',
      );

      if (kIsWeb) {
        appDebugLog('🌐 getFcmToken: Web platform - using VAPID key');
        _currentToken = await _firebaseMessaging.getToken(
      ...
      appDebugLog(
        '📊 getFcmToken: Token result: ${_currentToken == null
            ? "NULL"
            : _currentToken!.isEmpty
            ? "EMPTY"
            : "Got token (${_currentToken!.length} chars)"}',
      );

      if (_currentToken != null && _currentToken!.isNotEmpty) {
        appDebugLog('✅ getFcmToken: Token obtained successfully');
        appDebugLog('🔑 Token preview: ${_currentToken!.substring(0, 20)}...');
        _tokenStreamController.add(_currentToken!);
      ...
      appDebugLog(
        '⚠️ getFcmToken: First attempt returned null/empty, retrying in 500ms...',
      );
      ...
      if (_currentToken != null && _currentToken!.isNotEmpty) {
        appDebugLog('✅ getFcmToken: Token obtained on retry');
        _tokenStreamController.add(_currentToken!);
      ...
      appDebugLog('❌ getFcmToken: Failed to obtain token after retry');
      appDebugLog(
        '💡 Possible reasons: Service worker not active, browser blocked permission, or network issue',
      );
      return '';
    } catch (e, stackTrace) {
      appDebugLog('❌ getFcmToken: Error getting FCM token: $e');
      appDebugLog('📍 Stack trace: $stackTrace');
      return '';
    }
  }
  ...
  Future<void> _sendTokenToServer() async {
    try {
      ...
      if (deviceId == null || deviceId.isEmpty) {
        if (kDebugMode) {
          appDebugLog('⚠️ Cannot send FCM token - device ID not available yet');
          appDebugLog('ℹ️ Token will be sent after device registration');
        }
        return;
      }

      if (kDebugMode) {
        appDebugLog('📤 Sending FCM token to server...');
      ...
      if (freshToken == null || freshToken.isEmpty) {
        if (kDebugMode) {
          appDebugLog(
            '⚠️ Cannot send FCM token - failed to fetch token from Firebase',
          );
        }
        return;
      }
      ...
      if (_lastSentToken == freshToken) {
        if (kDebugMode) {
          appDebugLog('⏭️ Skipping API call - token already sent to server');
        }
        return;
      }
      ...
      if (kDebugMode) {
        appDebugLog(
          '   Token (fresh from Firebase): ${freshToken.substring(0, 20)}...',
        );
      }
      ...
      if (kDebugMode) {
        appDebugLog('✅ FCM token successfully sent to server');
      }
    } catch (e) {
      if (kDebugMode) {
        appDebugLog('❌ Error sending FCM token to server: $e');
        appDebugLog(
          'ℹ️ This is not critical - token will be retried on next refresh',
        );
      }
    }
  }
...
Future<void> firebaseMessagingBackgroundHandler(RemoteMessage message) async {
  if (kDebugMode) {
    appDebugLog('📨 Background message received:');
    appDebugLog('  Title: ${message.notification?.title}');
    appDebugLog('  Body: ${message.notification?.body}');
    appDebugLog('  Data: ${message.data}');
  }
