...
class ApiService {
  ...
  void initialize() {
    if (_isInitialized) {
      appDebugLog('API service already initialized, skipping...');
      return;
    }

    appDebugLog('Initializing API service... base url:${ApiConstants.baseUrl}');
    _dio = Dio(
      BaseOptions(
    ...
    _dio!.interceptors.add(
      InterceptorsWrapper(
        onRequest: (options, handler) {
          appDebugLog('Request URL: ${options.uri}');
    ...
    _isInitialized = true;
    appDebugLog('API service initialized successfully');
  }
  ...
  void setupDioLogging() {
    appDebugLog('setupDioLogging called');
    if (_dio != null) {
      _dio!.interceptors.add(
        LogInterceptor(
          ...
          logPrint: kDebugMode ? (obj) => appDebugLog(obj) : (obj) {},
        ),
      );
    }
  }
  ...
  Future<Map<String, dynamic>> getProductRelatedData({
    ...
    try {
      ...
      if (orderTypeId != null) {
        queryParams = {'branch_id': branchId, 'ordertypeid': orderTypeId};
      } else {
      ...
      appDebugLog('getProducts:${jsonEncode(response.data)}');
      ...
      setupDioLogging();
      appDebugLog('DioException during fetch product: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('Error fetching product related data: $e');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> registerGuestUserApiCall({
    ...
    appDebugLog('registerGuestUserApiCall');
    appDebugLog('Device ID: $deviceId');
    try {
      _ensureInitialized();

      appDebugLog('Making POST request to ${ApiConstants.guestUserRegister}');
      var data = {"device": deviceId, "login_type": "from web"};
      appDebugLog('registerGuestUserApiCall data: ${data}');
      final response = await _dio!.post(
        ApiConstants.guestUserRegister,
        data: data,
      );
      appDebugLog('registerGuestUserApiCall data: ${response}');
      if (response.statusCode == 200) {
        return response.data;
      } else {
        throw Exception(
          'Failed to register guest user: ${response.statusCode}',
        );
      }
    } on DioException catch (e) {
      appDebugLog('DioException during guest user registration: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('Error registering guest user: $e');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> refreshTokenApiCall() async {
    appDebugLog('🔄 refreshTokenApiCall - calling refresh token endpoint');

    try {
      _ensureInitialized();

      appDebugLog('Making POST request to ${ApiConstants.refreshToken}');
      ...
      if (response.statusCode == 200) {
        appDebugLog('✅ Refresh token API call successful');
        return response.data;
      } else {
        throw Exception('Failed to refresh token: ${response.statusCode}');
      }
    } on DioException catch (e) {
      appDebugLog('❌ DioException during token refresh: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('❌ Error refreshing token: $e');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> addUserFcmToken({
    ...
    appDebugLog('addUserFcmToken called');
    try {
      _ensureInitialized();
      var bodyParams = {
        "device": deviceId,
      ...
      appDebugLog(
        'Making POST request to ${ApiConstants.addUserFcm}::${bodyParams}',
      );
      final response = await _dio!.post(
      ...
      if (response.statusCode == 200) {
        appDebugLog('FCM token added successfully: ${response.data}');
        return response.data;
      } else {
        throw Exception('Failed to add FCM token: ${response.statusCode}');
      }
    } on DioException catch (e) {
      appDebugLog('DioException during FCM token addition: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
  ...
  Future<TableListResponse> getTableList({required String branchId}) async {
    try {
      ...
      appDebugLog('Fetching table list for branch: $branchId');
      final response = await _dio!.get(
        'getTableList',
        queryParameters: {'branch_id': branchId},
      );

      appDebugLog('Table list API response status: ${response.statusCode}');

      if (response.statusCode == 200) {
        appDebugLog('Table list API response data: ${response.data}');
        return TableListResponse.fromJson(response.data);
      } else {
        throw Exception('Failed to load table list: ${response.statusCode}');
      }
    } on DioException catch (e) {
      appDebugLog('DioException during table list fetch: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('Error fetching table list: $e');
      rethrow;
    }
  }
  ...
  Future<OrderTypesResponse> getOrderTypes() async {
    try {
      ...
      appDebugLog('Fetching order types...');
      final response = await _dio!.get(ApiConstants.getOrderTypes);

      appDebugLog('Order types API response status: ${response.statusCode}');

      if (response.statusCode == 200) {
        appDebugLog('Order types API response data: ${response.data}');
        return OrderTypesResponse.fromJson(response.data);
      } else {
        throw Exception('Failed to load order types: ${response.statusCode}');
      }
    } on DioException catch (e) {
      appDebugLog('DioException during order types fetch: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('Error fetching order types: $e');
      rethrow;
    }
  }
  ...
  Future<BranchListResponse> getBranchList() async {
    try {
      ...
      appDebugLog('Fetching branch list...');
      final response = await _dio!.get(ApiConstants.getBranchList);

      appDebugLog('Branch list API response status: ${response.statusCode}');

      if (response.statusCode == 200) {
        appDebugLog('Branch list API response data: ${response.data}');
        return BranchListResponse.fromJson(response.data);
      } else {
        throw Exception('Failed to load branch list: ${response.statusCode}');
      }
    } on DioException catch (e) {
      appDebugLog('DioException during branch list fetch: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
  ...
  Future<CreateOrderResponseModel> createOrder({
    ...
    try {
      ...
      appDebugLog('Creating order...');
      appDebugLog('Order request data: ${requestData.toJson()}');
      ...
      appDebugLog('Create order API response status: ${response.statusCode}');
      appDebugLog('Create order API response data: ${response.data}');
      ...
      appDebugLog('DioException during order creation: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('Error creating order: $e');
      rethrow;
    }
  }
  ...
  Future<CustomerAddResponse> addCustomer({
    ...
    try {
      ...
      appDebugLog('Adding customer...');
      appDebugLog('Customer request data: ${request.toJson()}');
      ...
      appDebugLog('Add customer API response status: ${response.statusCode}');
      appDebugLog('Add customer API response data: ${response.data}');
      ...
      appDebugLog('DioException during customer addition: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('Error adding customer: $e');
      rethrow;
    }
  }
  ...
  Future<bool> checkProductAvailability({required int productId}) async {
    try {
      ...
      appDebugLog('Checking product availability for product ID: $productId');
      ...
      appDebugLog(
        'Check product availability API response status: ${response.statusCode}',
      );
      appDebugLog(
        'Check product availability API response data: ${response.data}',
      );
      ...
      appDebugLog(
        'DioException during product availability check: ${e.message}',
      );
      ...
      if (e.response?.statusCode == 400) {
        ...
        appDebugLog('e.response:${e.response}');
        appDebugLog('e.message:${e.message}');
        throw Exception(
      ...
      appDebugLog('Error checking product availability: $e');
      throw Exception('Unable to check item availability. Please try again.');
    }
  }
    ...
    try {
      ...
      if (response.statusCode == 200) {
        if (response.data is Map<String, dynamic>) {
          return CartPriceSyncResponseDto.fromJson(
  ...
  Future<UserOrdersResponse> getUserOrders() async {
    try {
      ...
      appDebugLog('Fetching user orders...');
      final response = await _dio!.get(ApiConstants.getUserOrders);

      appDebugLog(
        'Get user orders API response status: ${response.statusCode}',
      );
      appDebugLog(
        'Get user orders API response data: ${jsonEncode(response.data)}',
      );
      ...
      appDebugLog('DioException during user orders fetch: ${e.message}');
      _handleDioError(e);
      rethrow;
    } catch (e) {
      appDebugLog('Error fetching user orders: $e');
      rethrow;
    }
  }
  ...
  Future<Map<String, dynamic>> cancelOrder({required int orderId}) async {
    try {
      ...
      appDebugLog('Cancel order API response status: ${response.statusCode}');
      appDebugLog('Cancel order API response data: ${response.data}');
      ...
      appDebugLog(
        'DioException during order cancellation: ${e.message},stataus code:${e.response?.statusCode},',
      );
      appDebugLog(
        'DioException during order cancellation data: ${e.response?.data},',
      );
  ...
  static void _handleDioError(DioException error) {
    appDebugLog('DioException details: ${error.toString()}');
    appDebugLog('Error type: ${error.type}');
    appDebugLog('Error message: ${error.message}');
    appDebugLog('Response data: ${error.response?.data}');
    appDebugLog('Response status code: ${error.response?.statusCode}');
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    if (_shouldRetry(err)) {
      ...
      if (retryCount < maxRetries) {
        appDebugLog(
          'Retrying request (${retryCount + 1}/$maxRetries) after ${retryDelays[retryCount]}',
        );
...
class TokenInterceptor extends Interceptor {
  ...
  void onRequest(
    ...
    appDebugLog('Request headers: ${options.headers}');
  ...
  void onError(DioException err, ErrorInterceptorHandler handler) async {
    ...
    if (err.response?.statusCode == 401) {
      ...
      appDebugLog('🔴 Received 401 error. Message: $message');
      ...
      final isFcmEndpoint = err.requestOptions.path.contains(
        ApiConstants.addUserFcm,
      );
      ...
      if (shouldRefreshToken) {
        ...
        try {
          ...
          appDebugLog('🔄 Calling refreshAccessToken...');
          ...
          if (refreshResponse) {
            ...
            appDebugLog('❌ Token refresh failed');
        ...
        if (isRefreshEndpoint) {
          appDebugLog('🔴 Refresh token endpoint failed. Clearing auth data.');
          await LocalStorage.clearAuthData();
        }
        return handler.next(err);
      }
    ...
    try {
      ...
      if (responseData['success'] == true && responseData['data'] != null) {
        ...
        if (newAccessToken != null && newRefreshToken != null) {
          ...
          appDebugLog('✅ Tokens saved successfully after refresh');
          return true;
        }
      }
      return false;
    } catch (e) {
      appDebugLog('❌ Token refresh failed: $e');
      return false;
    }
  }
  ...
  Future<void> _retryRequest(
    ...
    try {
      appDebugLog('🔄 Retrying original request: ${requestOptions.path}');
      ...
      appDebugLog('❌ Error retrying request: $e');
      return handler.reject(
  ...
  Future<void> _waitForTokenRefresh() async {
    ...
    if (attempts >= maxAttempts) {
      appDebugLog('⚠️ Token refresh wait timeout');
    }
  }
